...
# ============================================================================
# Takes the same extracted content summary as 03_draft_thread.R and drafts a
# single long-form LinkedIn post. Both links (article URL + X profile) live in
# the post body per prompts/linkedin_post_system.md. Reuses call_claude() from
# 03_draft_thread.R.
# ============================================================================
...
draft_linkedin_post <- function(content_summary) {
  ...
  user_prompt <- glue("
Generate a LinkedIn post for this Substack piece.
...
Return ONLY a JSON object with \"post_text\" and \"notes\" fields.
No text outside the JSON object.
")

  raw_response <- call_claude(system_prompt, user_prompt, max_tokens = 2000)
...
parse_linkedin_response <- function(raw_response, post_link) {
  ...
  if (is.null(parsed$post_text) || !nzchar(parsed$post_text)) {
  ...
  # Both links belong in the body (explicit Merrittocracy call — see prompt).
  # If the model dropped one, append the sign-off block rather than fail.
  if (!str_detect(post_text, fixed(post_link)) ||
      !str_detect(post_text, fixed(X_PROFILE_URL))) {
    cli_alert_warning("Sign-off links incomplete — appending")
    post_text <- post_text |>
      str_remove_all("https?://\\S*substack\\S*") |>
      str_remove(fixed(X_PROFILE_URL)) |>
      str_trim() |>
      paste0("\n\n", X_PROFILE_URL, "\n", post_link)
  }
