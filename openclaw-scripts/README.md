...
### `x-feedback-send.R` / `x-feedback-send.sh`
Sends a single x-monitor candidate plus its drafted reply to Telegram
with three inline feedback buttons: **Skip**, **As-is**, **Edited**.
Reads `{"candidate": {...}, "draft": "..."}` from stdin. Appends a
record to `data/x-monitor-surfacings.jsonl` capturing tweet ID, tier,
draft style, draft text, and the resulting Telegram message ID.
...
### `x-feedback-poll.R` / `x-feedback-poll.sh`
Polls Telegram's `getUpdates` endpoint for callback_query events
generated by the feedback buttons above. Logs each click to
`data/x-monitor-feedback.jsonl` (bucket = skip / posted_clean /
posted_edited, plus tweet ID and timestamps) and answers the callback
so the Telegram button clears its loading spinner. Tracks the update
offset in `data/x-feedback-poller-state.json`.

Called by an OpenClaw cron job every minute during waking hours.
Telegram requires answering a callback_query within ~30 seconds, so
the cron cadence cannot be slower than 1 minute without UX regression.
...
- `~/autopilot/data/x-monitor-surfacings.jsonl` — append log of each
  candidate sent to Telegram with feedback buttons.
- `~/autopilot/data/x-monitor-feedback.jsonl` — append log of each
  feedback bucket recorded by `x-feedback-poll`.
- `~/autopilot/data/x-feedback-poller-state.json` — Telegram update
  offset for the feedback poller.
